document.addEventListener("DOMContentLoaded", async () => {
 ...
 document.getElementById('back-to-anime').href = `${window.resolveSitePath('/pages/anime.html')}?id=${animeId}`;
 ...
 if (apiResponse && apiResponse.data) {
  ...
  const renderEpisodePicker = () => {
   ...
   episodePickerList.querySelectorAll('.episode-chip').forEach(button => {
    button.addEventListener('click', () => {
     ...
     if (!isNaN(chosenEp)) {
      window.location.href = `${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${chosenEp}`;
     }
    });
   });
  ...
  prevEpisodeBtn.onclick = () => {
   if (currentEp > 1) {
    ...
    window.location.href = `${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${previousEp}`;
   }
  };
  ...
  nextEpisodeBtn.onclick = () => {
   ...
   if (currentEp < maxEp) {
    ...
    window.location.href = `${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${nextEp}`;
   }
  };
 ...
 epPicker.onchange = (e) => {
  window.location.href = `${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${e.target.value}`;
 };
