  ...
  document.title = `${cleanTitle} - Omega-Roll`;
    ...
    const appendEpisodes = (list) => {
     list.forEach(ep => {
        ...
        li.innerHTML = `<a href="${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${epNumber}" title="${titleLabel}">${titleLabel} ${badges.join(' ')}</a>`;
    ...
    if (episodesData.length > 0) {
     ...
     for (let i = 1; i <= releasedCount; i++) {
        const li = document.createElement('li');
        li.className = 'episode-item';
        li.innerHTML = `<a href="${window.resolveSitePath('/pages/watch.html')}?animeId=${animeId}&ep=${i}" title="Episode ${i}">Episode ${i}</a>`;
        episodeListContainer.appendChild(li);
     }
    }
